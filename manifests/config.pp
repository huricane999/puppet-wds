...
class wds::config (
  ...
  if $::wds_conf {
    if !has_key($::wds_conf, 'installation_state') or !has_key($::wds_conf['installation_state'], 'remoteinstall_location') or $remote_install_path != $::wds_conf['installation_state']['remoteinstall_location'] {
